...
if secrets_auth._pat is None:
    secrets_auth._pat = load_pat()
...
if (
    module_query_params.get("pat", "") == ""
    and module_query_params.get("token", "") == ""
):
    unauthorized = True
else:
    unauthorized = False
...
API_INFO = list_all_models(filter_by={"query": "LLM"})

default_llms = get_default_models()

st.markdown("# LLM Battleground")
...
def run_model(input_text, model):
    m = API_INFO[model]
    user_model_obj = Model(
        model_id=m["model_id"],
        user_id=m["user_id"],
        app_id=m["app_id"],
        pat=secrets_auth.pat,
    )
    while True:
        try:
            response = user_model_obj.predict_by_bytes(
                bytes(input_text, "utf-8"), "text"
            )
...
models = st.multiselect(
    "Select the LLMs you want to use:",
    model_names,
...
def render_card(container, input, caller_id, completions):
    ...
    for d in completions:
        ...
        container.markdown(f"Completed in {round(d['duration']):,}ms")
...
if st.session_state["generated_completions"]:
    if inp and models:
        ...
        for mi, model in enumerate(models):
            ...
            m = API_INFO[model]
            model_url = h.clarifai_url(
                m["user_id"], m["app_id"], "models", m["model_id"]
            )
            model_url_with_version = h.clarifai_url(
                m["user_id"], m["app_id"], "models", m["model_id"], m["version_id"]
            )
        ...
        edited_df = st.data_editor(
            c, disabled=set(completions[0].keys()) - set(["select"])
        )
...
with st.expander("Recent Messages from Others"):
    ...
    def completions_for_input(input_id):
        ...
        for completion_hit in completion_search_response.hits:
            if (
                completion_hit.input.data.metadata.fields["input_id"].string_value
                == input_id
            ):
                txt = get_text(secrets_auth, completion_hit.input.data.text.url)
                model_url = completion_hit.input.data.metadata.fields[
                    "model"
                ].string_value
